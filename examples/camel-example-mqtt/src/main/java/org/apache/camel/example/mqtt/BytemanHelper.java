...
public class BytemanHelper {
    ...
    public void traceClientSend(MQTTFrame frame, UTF8Buffer clientId) {
        if (DISCONNECT.TYPE == frame.messageType()) {
            //printStackTrace();
        }
    ...
    public void traceNewConnection(DispatchQueue queue) {
        StringBuilder str = new StringBuilder("@>>>>> client-nwcn: queue=").append(queue);
        _logger.trace(str.toString());
    }

    public void traceStartEndpoint(CallbackConnection connection) {
        StringBuilder str = new StringBuilder("@>>>>> camel-stred: queue=").append(connection.getDispatchQueue());
        _logger.trace(str.toString());
    }
